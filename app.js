...
// === TRACKING FOR VIDEO PLAY LENGTH AND SCROLLING DEPTH ===

document.addEventListener("DOMContentLoaded", () => {
  // === VIDEO TRACKING for EverAfterPromo.mp4 ===
  const video = document.querySelector('video[src*="EverAfterPromo.mp4"]');
  if (video && typeof gtag === "function") {
    let videoMilestones = { 10: false, 25: false, 50: false, 75: false };
    ...
    video.addEventListener("timeupdate", () => {
      ...
      [10, 25, 50, 75].forEach(threshold => {
        if (!videoMilestones[threshold] && percent >= threshold) {
          gtag("event", `video_progress_${threshold}`, {
            video_title: "EverAfterPromo.mp4",
          });
          videoMilestones[threshold] = true;
        }
      });
    });
  }
  ...
  const scrollPoints = [12, 26, 36, 42, 49, 62, 67, 86, 97];
  const scrollFired = {};
